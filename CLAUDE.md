...
```json
{
  ...
  "color_map": [[50.2, "#BA01FF"], [95.0, "#3FD34A"], [135.0, "#FC284F"], ...],
...
`color_map` is a knot table over θ, **not** a table of zones. Eight knots are the calibrated palette, one per zone. The ninth, at θ=95°, belongs to no zone: θ 75–120° is the densest region of the corpus (~250 tracks, all AcousticBrainz-sourced) and the widest gap in the table, and interpolated straight from fun/dance to industrial it passed within ΔE 6 of shoegaze — 15% of the library reading as a zone it was not. Green is the one region of Lab the palette leaves open.
